...
describe('Products SDK Tests', () => {
  ...
  it('Verify parameter mapping for analytics_product_daily_trend', async () => {
    ...
    const res = await products.dailyTrend({
      productId: 8516,
      startDate: '2026-08-01',
      endDate: '2026-08-04',
      companyName: 'Horeca Smart',
    ...
    expect(supabase!.rpc).toHaveBeenCalledWith('analytics_product_daily_trend', {
      p_product_id: 8516,
      p_start_date: '2026-08-01',
      p_end_date: '2026-08-04',
      p_company_name: 'Horeca Smart',
  ...
  it('Verify parameter mapping for analytics_product_company_split', async () => {
    ...
    const res = await products.companySplit({
      productId: 8516,
      startDate: '2026-08-01',
      endDate: '2026-08-04',
    });

    expect(supabase!.rpc).toHaveBeenCalledWith('analytics_product_company_split', {
      p_product_id: 8516,
      p_start_date: '2026-08-01',
      p_end_date: '2026-08-04',
    });
  ...
  it('Verify parameter mapping for analytics_product_lifecycle', async () => {
    (supabase!.rpc as any).mockResolvedValueOnce({
      data: [{
        ...
        lifetime_sales: 7920429.00,
        active_months: 8,
        average_days_between_sales: 2.1,
      }],
      error: null,
      count: null,
      status: 200,
    ...
    expect(supabase!.rpc).toHaveBeenCalledWith('analytics_product_lifecycle', {
      p_product_id: 8516,
    });
  ...
  it('Verify parameter mapping for analytics_product_data_quality', async () => {
    ...
    expect(supabase!.rpc).toHaveBeenCalledWith('analytics_product_data_quality', {
      p_product_id: 8516,
    });
  ...
  it('Verify parameter mapping for analytics_product_alerts', async () => {
    (supabase!.rpc as any).mockResolvedValueOnce({
      data: [{
        alert_code: 'NO_RECENT_SALES',
        severity: 'high',
        title_ar: 'توقف المبيعات المؤخرة',
        details_ar: 'لم يتم تسجبل مبيعات للمنتج منذ أكثر من 30 يوماً',
        metric_value: 35,
      }],
      error: null,
      count: null,
    ...
    const res = await products.alerts({
      productId: 8516,
      asOfDate: '2026-08-04',
      companyName: null,
    });

    expect(supabase!.rpc).toHaveBeenCalledWith('analytics_product_alerts', {
      p_product_id: 8516,
      p_as_of_date: '2026-08-04',
      p_company_name: null,
    });
  ...
  it('Verify parameter mapping for analytics_product_score', async () => {
    (supabase!.rpc as any).mockResolvedValueOnce({
      data: [{
        sales_strength_score: 35,
        growth_score: 18,
        coverage_score: 15,
        consistency_score: 8,
        data_quality_score: 7,
        total_score: 83,
        methodology_version: 'P1_v1',
      }],
      error: null,
      count: null,
      status: 200,
    ...
    const res = await products.score({
      productId: 8516,
      startDate: '2026-08-01',
      endDate: '2026-08-04',
      companyName: null,
    ...
    expect(supabase!.rpc).toHaveBeenCalledWith('analytics_product_score', {
      p_product_id: 8516,
      p_start_date: '2026-08-01',
      p_end_date: '2026-08-04',
      p_company_name: null,
  ...
  it('Verify parameter mapping for analytics_product_reconciliation', async () => {
    (supabase!.rpc as any).mockResolvedValueOnce({
      data: [{
        order_sales: 3704906.00,
        product_line_sales: 3704906.00,
        difference_value: 0,
    ...
    const res = await products.reconciliation({
      startDate: '2026-08-01',
      endDate: '2026-08-04',
    });

    expect(supabase!.rpc).toHaveBeenCalledWith('analytics_product_reconciliation', {
      p_start_date: '2026-08-01',
      p_end_date: '2026-08-04',
      p_company_name: null,
    });
  ...
  it('Verify parameter mapping for analytics_product_customer_retention', async () => {
    (supabase!.rpc as any).mockResolvedValueOnce({
      data: [{
        customer_id: 101,
        customer_name: 'Super Market Cairo',
        company_name: 'Horeca Smart',
        primary_salesperson: 'Ahmed Hassan',
        previous_orders: 8,
        current_orders: 0,
        previous_quantity: 120,
        current_quantity: 0,
        previous_sales: 218050.50,
        current_sales: 0,
        sales_change_pct: -100.0,
        previous_last_order: '2026-06-15',
        current_last_order: null,
    ...
    const res = await products.customerRetention({
      productId: 8516,
    ...
    expect(supabase!.rpc).toHaveBeenCalledWith('analytics_product_customer_retention', {
      p_product_id: 8516,
  ...
  it('Verify parameter mapping for analytics_product_customer_retention_summary', async () => {
    (supabase!.rpc as any).mockResolvedValueOnce({
      data: [{
        previous_customers: 20,
        current_customers: 24,
        retained_customers: 4,
        new_customers: 14,
        stopped_customers: 10,
        declining_customers: 6,
        previous_sales: 949455.50,
        current_sales: 3266067.50,
        stopped_sales_opportunity: 218050.50,
        declining_sales_gap: 90840.00,
    ...
    const res = await products.customerRetentionSummary({
      productId: 8516,
      startDate: '2026-07-01',
      endDate: '2026-07-31',
      companyName: null,
    ...
    expect(supabase!.rpc).toHaveBeenCalledWith('analytics_product_customer_retention_summary', {
      p_product_id: 8516,
      p_start_date: '2026-07-01',
      p_end_date: '2026-07-31',
      p_company_name: null,
