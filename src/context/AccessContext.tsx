...
export const AccessProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const resolveProfile = useCallback(async (activeSession: Session) => {
    ...
    try {
      const nextProfile = await getCurrentAccessProfile();
      ...
      setError(err instanceof Error ? err.message : 'ACCESS_PROFILE_UNAVAILABLE');
    }
  }, []);
  ...
  const clearSession = useCallback(() => {
    ...
    setProfile(null);
    setError(null);
    setStatus(isSupabaseConfigured ? 'unauthenticated' : 'not_configured');
  }, []);
  ...
  useEffect(() => {
    ...
    getCurrentSession()
      .then((initialSession) => {
        ...
        return resolveProfile(initialSession);
      })
      .catch((err) => {
        if (!active) return;
        setError(err instanceof Error ? err.message : 'AUTH_SESSION_ERROR');
        setStatus('unauthorized');
      });
  ...
  const signIn = useCallback(async (email: string, password: string) => {
    ...
    try {
      ...
      await resolveProfile(auth.session);
    } catch (err) {
      clearSession();
  ...
  }, [clearSession, resolveProfile, session]);
  ...
  const value = useMemo<AccessContextValue>(() => ({
    ...
    signOut,
    refreshProfile,
  }), [status, user, session, profile, error, signIn, signOut, refreshProfile]);
