...
describe('P0 Audit: Non-Existence of Fake / Mock / Demo Data', () => {
  it('should export empty arrays for all mockData exports', () => {
    expect(mockData.INITIAL_KPIS).toEqual([]); expect(mockData.SALES_REPS).toEqual([]); expect(mockData.CUSTOMERS).toEqual([]); expect(mockData.PRODUCTS).toEqual([]); expect(mockData.CATEGORIES).toEqual([]); expect(mockData.AREAS).toEqual([]); expect(mockData.LOST_CUSTOMERS).toEqual([]); expect(mockData.RECENT_ORDERS).toEqual([]); expect(mockData.MONTHLY_REVENUE_TREND).toEqual([]);
  });
  it('should not contain known fake order IDs anywhere in mockData', () => {
    const serialized = JSON.stringify(mockData); expect(serialized).not.toContain('ORD-2026-8801'); expect(serialized).not.toContain('ORD-2026-8802'); expect(serialized).not.toContain('ORD-2026-8803'); expect(serialized).not.toContain('ORD-2026-8804');
  });
  it('should not contain known fake entity names in mockData', () => {
    const serialized = JSON.stringify(mockData); expect(serialized).not.toContain('ريتز كارلتون'); expect(serialized).not.toContain('مكة أوركيد'); expect(serialized).not.toContain('أورورا أرتيزان'); expect(serialized).not.toContain('التموين الملكي'); expect(serialized).not.toContain('طارق الغامدي'); expect(serialized).not.toContain('ريم الخطيب'); expect(serialized).not.toContain('خالد فاروق');
  });
